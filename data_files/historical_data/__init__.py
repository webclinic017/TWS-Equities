...
def generate_failure_dataframe(location, debug):
    """
        # TODO: to be added...
        @param debug:
        @param location:
...
def generate_success_dataframe(location, debug):
    """
        # TODO: to be added...
        @param debug:
        @param location:
...
def create_csv_dump(target_date, debug=False):
    """
        TODO: To be added...
    """
    ...
    if isdir(success_location):
        data_frame = generate_success_dataframe(success_location, debug)
        data_frame.to_csv(f'{target_location}/success.csv')
        if debug:
            sys.stdout.write(f'\nSuccess CSV: {join(target_location, "success.csv")}\n')
    if isdir(failure_location):
        data_frame = generate_failure_dataframe(failure_location, debug)
